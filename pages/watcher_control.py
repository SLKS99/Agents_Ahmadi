...
if watch_path.exists() and watch_path.is_dir():
    st.success(f"✅ Watch directory exists and is accessible")
    try:
    ...
    st.warning(f"⚠️ Watch directory does not exist or is not accessible: `{watcher_directory}`")
...
try:
    ...
    if response.status_code == 200:
        ...
        st.success(f"✅ Watcher server is running")
        
        observer_running = health_data.get("observer_running", False)
        watch_dir = health_data.get("watch_dir")
        
        if observer_running:
            st.success("👁️ **File system observer is ACTIVE** - Ready to detect files!")
            if watch_dir:
                st.info(f"📁 Watching directory: `{watch_dir}`")
                st.caption("💡 Add CSV/Excel files to this directory to trigger curve fitting")
        else:
            st.warning("⚠️ **Observer is NOT running**")
            st.info("👉 Click '▶️ Start Watching' below to begin monitoring your directory")
            if watch_dir:
    ...
    if "watcher_server_process" in st.session_state:
        ...
        if process.poll() is None:
            ...
            st.warning("⚠️ Server process exists but not responding. Try stopping and restarting.")
        else:
            # Process has died
            st.warning("⚠️ Server process has stopped. You can start it again below.")
            if "watcher_server_process" in st.session_state:
                del st.session_state.watcher_server_process
            if "watcher_server_pid" in st.session_state:
                del st.session_state.watcher_server_pid
        ...
        if st.button("🚀 Start Server", use_container_width=True, type="primary", key="start_server_btn"):
            ...
            try:
                ...
                if test_response.status_code == 200:
                    st.warning("⚠️ Port 8000 is already in use. Server may already be running.")
...
with col_server2:
    if server_running:
        ...
        st.button("⏹️ Server Stopped", use_container_width=True, disabled=True)
...
if server_running:
    st.subheader("👁️ Watching Control")
    
    col1, col2, col3 = st.columns(3)

    with col1:
        if st.button("▶️ Start Watching", use_container_width=True, type="primary", key="start_watching_btn"):
            try:
                ...
                if response.status_code == 200:
                    ...
                    st.success(f"✅ Started watching directory!")
    ...
    with col2:
        if st.button("⏹️ Stop Watching", use_container_width=True, key="stop_watching_btn"):
            try:
    ...
    with col_scan1:
        if st.button("🔍 Scan Directory for Files", use_container_width=True, key="scan_directory_btn"):
            try:
                ...
                if response.status_code == 200:
                    result = response.json()
                    st.success(f"✅ Scanned directory successfully!")
    ...
    st.info("💡 Start the watcher server above to enable watching controls.")
...
with st.expander("📖 How to Use the Watcher", expanded=True):
    ...
    3. **Configure Watch Directory** (if needed):
       - Go to Settings → Watcher tab
       - Set "Watcher Directory" to the folder you want to monitor
       - Default: Your OneDrive data folder
    
    4. **Start Watching**:
       - Click "▶️ Start Watching" above
       - The watcher will monitor the directory for CSV/Excel files
...
# Log Viewer Section
st.subheader("Watcher Server Logs")
...
if watch_path.exists():
    st.subheader("📁 Watch Directory Contents")
